...
function BibleTextPicker({
	...
	const [book, setBook] = useState(bibleCv?.book ? bibleCv?.book?.name : books[0]);
	...
	useEffect(() => {
		if (bibleCv && bibleCv?.success) {
			setActualValue((prev) => (bibleCv?.success ? bibleCv?.toShortString() : prev));
	...
	return (
		<>
			...
			>
				...
				<FieldInput ref={verseInputRef} id={name} value={actualValue} onChange={onChange}></FieldInput>
				<FieldHint />
				<FieldError />
			</Field>
			<Stack horizontal spacing={4}>
				...
				<Box>
					<Select id="chapter" label="Chapter" value={chapter} onChange={handleChapterChange}>
				...
				<Box>
					<Field id="verse">
						...
						<FieldInput id="verse" value={verse} onChange={handleVerseChange}></FieldInput>
					</Field>
				</Box>
			</Stack>
...
export default BibleTextPicker;
